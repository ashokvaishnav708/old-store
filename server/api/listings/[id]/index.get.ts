  ...
  const listing = await cached(`listing:${id}`, 30, async () => {
    const [row] = await db
      .select({
        ...
        topPlacementBoost: sql<boolean>`(${listings.topPlacementBoost} AND ${listings.boostsExpireAt} > now())`,
        homepageBoost: sql<boolean>`(${listings.homepageBoost} AND ${listings.boostsExpireAt} > now())`,
        boostsExpireAt: listings.boostsExpireAt,
